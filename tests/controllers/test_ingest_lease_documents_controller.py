...
class TestIngestLeaseDocumentsControllerBase(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_ingestion_site_audit_lease_service = Mock(spec=IngestionCollectionDocumentService)
        ...
        self.controller = IngestLeaseDocumentsController(
            ...
            ingestion_site_audit_lease_service=self.mock_ingestion_site_audit_lease_service,
...
class TestIngestDocuments(TestIngestLeaseDocumentsControllerBase):
    def test_when_not_ingested_returns_correct_response_analyzer_only(self):
        ...
        self.mock_ingestion_site_audit_lease_service.is_lease_document_ingested.return_value = False
        ...
        self.assertEqual(self.mock_ingestion_site_audit_lease_service.ingest_analyzer_output.call_count, len(documents))

        for document in documents:
            self.mock_content_understanding_client.begin_analyze_data.assert_any_call(
                "test-analyzer", document.file_bytes
            )
            self.mock_ingestion_site_audit_lease_service.ingest_analyzer_output.assert_any_call(
                IngestDocumentType.SITE,
                document.market,
                document.id,
                document.lease_id,
                document.filename,
    ...
    def test_when_already_ingested_returns_correct_response_analyzer_only(self):
        ...
        self.mock_ingestion_site_audit_lease_service.is_lease_document_ingested.return_value = True
        self.mock_ingestion_configuration_management_service.load_config.return_value = mock_config
        self.mock_content_understanding_client.begin_analyze_data.return_value = Mock()
        self.mock_content_understanding_client.poll_result.return_value = mock_analyzer_output
        ...
        self.mock_ingestion_site_audit_lease_service.ingest_analyzer_output.assert_not_called()
    ...
    def test_when_not_ingested_returns_correct_response_classifier_enabled(self):
        ...
        self.assertEqual(self.mock_content_understanding_client.poll_result.call_count, len(documents))
        classifier_output_call_count = self.mock_ingestion_site_audit_lease_service.ingest_classifier_output.call_count
        ...
        for document in documents:
            ...
            self.mock_ingestion_site_audit_lease_service.ingest_classifier_output.assert_any_call(
                IngestDocumentType.SITE,
                document.market,
                document.id,
                document.lease_id,
    ...
    def test_when_already_ingested_returns_correct_response_classifier_enabled(self):
        ...
        self.mock_content_understanding_client.begin_analyze_data.assert_not_called()
        self.mock_content_understanding_client.poll_result.assert_not_called()
        self.mock_ingestion_site_audit_lease_service.ingest_classifier_output.assert_not_called()
        self.mock_ingestion_site_audit_lease_service.ingest_analyzer_output.assert_not_called()
    ...
    def test_when_classifier_disabled_uses_analyzer_path(self):
        ...
        self.mock_ingestion_site_audit_lease_service.ingest_analyzer_output.assert_called_once_with(
            IngestDocumentType.SITE,
            documents[0].market,
            documents[0].id,
