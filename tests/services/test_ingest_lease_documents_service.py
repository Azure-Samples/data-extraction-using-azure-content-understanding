...
class TestIngestionCollectionDocumentServiceIngestAnalyzerOutput(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_site_audit_collection = MagicMock()
        self.mock_mongo_lock_manager = MagicMock()

        self.service = IngestionCollectionDocumentService(
            site_audit_collection=self.mock_site_audit_collection,
    ...
    def test_ingest_analyzer_output_success(self, mock_logging):
        ...
        self.mock_site_audit_collection.find_one.assert_called_once_with(
            {"_id": "test_site-fake_hash"}
        )
        ...
        self.mock_container_client.upload_document.assert_called_once_with(
            "some_markdown",
            "Sites/market 1/test_site/test_lease/test_file.md"
        )
        mock_logging.info.assert_called_with(
            "Data ingested from analyzer output successfully for site_id=test_site, "
            "lease_id=test_lease, lease_config_hash=fake_hash"
        )
        self.mock_site_audit_collection.update_one.assert_called_once_with(
            {"_id": "test_site-fake_hash"},
            {
                "$set":
                {
                        ...
                        "leases": [
                            {
                                "lease_id": "test_lease",
                                "original_documents": [
                                    "Sites/market 1/test_site/test_lease/test_file.pdf"
                                ],
                                "markdowns": [
                                    "Sites/market 1/test_site/test_lease/test_file.md"
                                ],
                                    ...
                                    "field1": [
                                        {
                                            ...
                                            "markdown": "Sites/market 1/test_site/test_lease/test_file.md",
                                            "document": "Sites/market 1/test_site/test_lease/test_file.pdf"
                                        }
                                    ],
                                    "field2": [
                                        {
    ...
    def test_ingest_analyzer_output_exception(self, mock_logging):
        data = {}
        self.mock_site_audit_collection.find_one.side_effect = Exception("DB error")
    ...
    def test_ingest_analyzer_output_with_existing_document(self, mock_logging):
        ...
        self.mock_site_audit_collection.find_one.return_value = existing_doc.model_dump()
        ...
        self.assertEqual(self.mock_site_audit_collection.find_one.call_count, 1)
        mock_logging.info.assert_called_with(
            "Data ingested from analyzer output successfully for site_id=test_site, "
...
class TestIngestionCollectionDocumentServiceCleanEmptyDocument(unittest.TestCase):
    ...
    def test_clean_empty_document_deletes_empty_document(self):
        document_id = "test_site-fake_hash"
        self.mock_site_audit_collection.find_one.return_value = {"_id": document_id}

        with patch("services.ingest_lease_documents_service.ExtractedCollectionDocuments", side_effect=Exception):
            self.service.clean_empty_document(site_id="test_site", config=self.config)

        self.mock_site_audit_collection.find_one.assert_called_once_with({"_id": document_id})
        self.mock_site_audit_collection.delete_one.assert_called_once_with({"_id": document_id})

    def test_clean_empty_document_no_action_for_valid_document(self):
        document_id = "test_site-fake_hash"
        valid_document = {"_id": document_id, "site_id": "test_site"}
        self.mock_site_audit_collection.find_one.return_value = valid_document

        with patch("services.ingest_lease_documents_service.ExtractedCollectionDocuments"):
            self.service.clean_empty_document(site_id="test_site", config=self.config)

        self.mock_site_audit_collection.find_one.assert_called_once_with({"_id": document_id})
        self.mock_site_audit_collection.delete_one.assert_not_called()
...
class TestIngestionCollectionDocumentServiceIsLeaseDocumentIngested(unittest.TestCase):
    ...
    def test_is_lease_document_ingested_returns_true(self):
        ...
        existing_document = ExtractedCollectionDocuments(
            ...
            information=ExtractedCollectionInformationCollection(
                leases=[
                    {
                        "lease_id": "test_lease",
                        "original_documents": ["Sites/market/test_site/test_lease/test_file.pdf"],
                        "markdowns": ["Sites/market/test_site/test_lease/test_file.md"],
                        "fields": {
                            "field1": [
                                {
                                    "type": "string",
        ...
        self.mock_site_audit_collection.find_one.return_value = existing_document.model_dump()
    ...
    def test_is_lease_document_ingested_returns_false_for_missing_site_document(self):
        ...
        self.mock_site_audit_collection.find_one.assert_called_once_with({"_id": "test_site-fake_hash"})
        ...
        existing_document = ExtractedCollectionDocuments(
            ...
            information=ExtractedCollectionInformationCollection(
                leases=[
                    {
                        "lease_id": "test_lease",
                        "original_documents": ["Sites/market/test_site/test_lease/other_file.pdf"],
                        "markdowns": ["Sites/market/test_site/test_lease/other_file.md"],
                        "fields": {
                            "field1": [
                                {
                                    "type": "string",
...
class TestIngestionCollectionDocumentServiceIngestClassifierOutput(unittest.TestCase):
    ...
    def test_ingest_classifier_output_success(self, mock_logging):
        ...
        mock_logging.info.assert_called_with(
            "Data ingested from classifier output successfully for site_id=test_site, "
            "lease_id=test_lease, lease_config_hash=fake_hash"
        )
        self.mock_site_audit_collection.update_one.assert_called_once_with(
            {"_id": "test_site-fake_hash"},
...
class TestIngestionCollectionDocumentServiceGetAllExtractedFields(unittest.TestCase):
    ...
    def test_get_all_extracted_fields_success_with_multiple_leases(self, mock_logging):
        ...
        result = self.service._get_all_extracted_fields_from_collection_doc("test_site", self.config)
        ...
        mock_logging.info.assert_called_with(
            "Querying CosmosDB for site ID test_site and Lease Config Hash test_hash"
        )
    ...
    def test_get_all_extracted_fields_no_document_found(self, mock_logging):
        ...
        result = self.service._get_all_extracted_fields_from_collection_doc("nonexistent_site", self.config)

        self.assertEqual(result, {})
        mock_logging.error.assert_called_with(
            "Error encountered while querying CosmosDB: "
            "data for site nonexistent_site and lease config hash test_hash does not exist."
        )

    @patch("services.ingest_lease_documents_service.logging")
    def test_get_all_extracted_fields_with_lease_documents(self, mock_logging):
        """Test extraction with fields that have is_lease=True."""
        ...
        lease_fields = {
            "field1": [
                ExtractedLeaseField(
                    ...
                    confidence=0.9,
                    date_of_document=date(2023, 1, 1),
                    document="test_LSE_doc.pdf"  # LSE document (is_lease=True)
                ),
                ExtractedLeaseField(
                    type=ExtractedLeaseFieldType.STRING,
                    ...
                    confidence=0.8,
                    date_of_document=date(2023, 6, 1),
                    document="test_AMD_doc.pdf"  # AMD document (is_lease=False)
                )
            ]
        }
        ...
        with patch.object(ExtractedLeaseField, 'is_lease', new_callable=lambda: property(
            lambda self: self.document and "_LSE_" in self.document
        )):
            result = self.service._get_all_extracted_fields_from_collection_doc("test_site", self.config)
    ...
    @patch("services.ingest_lease_documents_service.logging")
    def test_get_all_extracted_fields_with_pruned_lease_documents(self, mock_logging):
        """Test extraction with fields that have is_lease=True."""
        # Create test data where some fields are from lease documents
        lease_fields = {
            "field1": [
                ...
                ExtractedLeaseField(
                    type=ExtractedLeaseFieldType.STRING,
                    valueString="amendment_value1",
                    confidence=0.7,
                    date_of_document=date(2023, 5, 1),
                    document="test_AMD_doc1.pdf"  # AMD document (is_lease=False)
                ),
                ExtractedLeaseField(
                    type=ExtractedLeaseFieldType.STRING,
                    valueString="amendment_value2",
                    confidence=0.8,
                    date_of_document=date(2023, 6, 1),
                    document="test_AMD_doc2.pdf"  # AMD document (is_lease=False)
                )
            ]
        }
        ...
        existing_document = ExtractedCollectionDocuments(
            ...
            information=ExtractedCollectionInformationCollection(
                leases=[
                    ExtractedLeaseCollection(
                        lease_id="test_lease",
                        original_documents=["test_LSE_doc.pdf", "test_AMD_doc1.pdf", "test_AMD_doc2.pdf"],
                        markdowns=["test_LSE_doc.md", "test_AMD_doc1.md", "test_AMD_doc2.md"],
                        fields=lease_fields
                    )
                ]
            )
    ...
    @patch("services.ingest_lease_documents_service.logging")
    def test_get_all_extracted_fields_site_id_case_handling(self, mock_logging):
        """Test that site_id is converted to uppercase when querying."""
        self.mock_site_audit_collection.find_one.return_value = None

        self.service._get_all_extracted_fields_from_collection_doc("lowercase_site", self.config)

        # Verify that the site_id was converted to uppercase in the query
        self.mock_site_audit_collection.find_one.assert_called_with(
            {"_id": "LOWERCASE_SITE-test_hash"}
        )

    @patch("services.ingest_lease_documents_service.logging")
    def test_get_all_extracted_fields_date_comparison(self, mock_logging):
        """Test that the method correctly selects the field with the latest date."""
        lease_fields = {
            "field1": [
                ExtractedLeaseField(
                    type=ExtractedLeaseFieldType.STRING,
                    valueString="older_value",
                    confidence=0.9,
                    date_of_document=date(2023, 1, 1),
                    document="older_doc.pdf"
                ),
                ExtractedLeaseField(
                    type=ExtractedLeaseFieldType.STRING,
                    valueString="newer_value",
                    confidence=0.8,
                    date_of_document=date(2023, 6, 1),  # Later date
                    document="newer_doc.pdf"
                )
            ]
        }
        ...
        existing_document = ExtractedCollectionDocuments(
            ...
            information=ExtractedCollectionInformationCollection(
                leases=[
                    ExtractedLeaseCollection(
                        lease_id="test_lease",
                        original_documents=["older_doc.pdf", "newer_doc.pdf"],
                        markdowns=["older_doc.md", "newer_doc.md"],
                        fields=lease_fields
                    )
                ]
            )
        ...
        with patch.object(ExtractedLeaseField, 'is_lease', new_callable=lambda: property(lambda self: False)):
            result = self.service._get_all_extracted_fields_from_collection_doc("test_site", self.config)

        # Should use the field with the later date
        lease_result = result["test_lease"]
        self.assertEqual(len(lease_result["field1"]), 1)

        # Verify it's the newer document by checking the model_dump contains the newer value
        field_data = lease_result["field1"][0]
        self.assertIsInstance(field_data, LeaseAgreementDocumentData)
        self.assertEqual(lease_result, {
            "field1": [
                LeaseAgreementDocumentData(
                    type=ExtractedLeaseFieldType.STRING,
    ...
    def test_get_all_extracted_fields_document_exists_but_invalid_structure(self, mock_logging):
        ...
        invalid_document = {"_id": "test_site-test_hash", "invalid_field": "invalid_data"}
        self.mock_site_audit_collection.find_one.return_value = invalid_document
        ...
        mock_logging.error.assert_called_with(
            "Error encountered while querying CosmosDB: "
            "data for site test_site and lease config hash test_hash does not exist."
        )
